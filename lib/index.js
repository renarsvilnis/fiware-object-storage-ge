...
module.exports = function (config) {
  ...
  lookupTenant(host, user, password)
    .then((tenant) => {
      tenant;
      return retrieveSecurityToken(host, tenant, user, password, region));
    .then((authData) => {
      // console.log(authData);
      return listContainer(authData.token, authData.url, container);
    })
    .then((items) => {
      console.log(items);
    });
  ...
  return {
    lookupTenant: funtion () {
      // if already has tenant fetched al
      if (tenant) {
        return Promise.resolve(tenant);
      }

      return lookupTenant(config.host, config.user, config.password)
        .then((newTenant) => {
          tenant = newTenant;
          return newTenant;
        });
